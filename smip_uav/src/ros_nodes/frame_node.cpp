...
FrameNode::FrameNode(const rclcpp::NodeOptions& options) : Node("frame_node", options) {
    // Set number of threads for parallelization
    omp_set_num_threads(4);

    // Declare and load parameters from launch file
    declare_parameters();
    load_parameters();
    ...
    cloud_sub_ = this->create_subscription<sensor_msgs::msg::PointCloud2>(
        cfg_.pointcloud_in_topic,
        rclcpp::SensorDataQoS(),
        std::bind(&FrameNode::pointcloud_callback, this, std::placeholders::_1)
    );
    cloud_repub_ = this->create_publisher<sensor_msgs::msg::PointCloud2>(
        cfg_.pointcloud_out_topic,
        rclcpp::SensorDataQoS()
    );

    pts_.reserve(cfg_.fbuild_cfg.tof_res_x * cfg_.fbuild_cfg.tof_res_y);

    RCLCPP_INFO(this->get_logger(), 
        "FrameNode initialized... \nListening for pointcloud stream on topic: %s",
        cfg_.pointcloud_in_topic.c_str()
    );
}

void FrameNode::declare_parameters() {
    ...
    cfg_.sensor_frame = this->get_parameter("sensor_frame").as_string();
    cfg_.pointcloud_in_topic = this->get_parameter("pointcloud_topic").as_string();
    cfg_.pointcloud_out_topic = this->get_parameter("pointcloud_republish_topic").as_string();
...
bool FrameNode::get_transform(const rclcpp::Time& stamp) {
    try {
        auto transform = tf_buffer_->lookupTransform(cfg_.odom_frame, cfg_.sensor_frame, stamp, rclcpp::Duration::from_nanoseconds(20'000'000));
...
void FrameNode::pointcloud_callback(sensor_msgs::msg::PointCloud2::SharedPtr cloud_msg) {
    clock_.tic();

    // Republish pointcloud (with correct frame)
    cloud_msg->header.frame_id = cfg_.sensor_frame;
    ...
    frame_ = fbuild_->process(pts_, timestamp_ns, &gnd);

    // Process the frame
    frame_surfels_ = fproc_->process(frame_);

    // Update buffer
    committed_surfels_ = fbuff_->push(frame_surfels_, tf_, timestamp_ns);

    // Publish evicted surfels

    double t_frame = clock_.toc();

    RCLCPP_INFO(this->get_logger(),
        "Frame processing time (total): %f - Surfels in Frame: %ld",
        t_frame, frame_surfels_.size()
    );
}
