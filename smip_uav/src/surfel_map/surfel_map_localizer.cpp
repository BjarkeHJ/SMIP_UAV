    ...
    const Eigen::Isometry3f& prior) 
{
    ...
    for (size_t iter = 0; iter < cfg_.max_iters; ++iter) {
        ...
        if (n_corrs < cfg_.min_inliers) {
            break; // Too few correspondences to true the solve -> bail
        }
    ...
    // Compute confidence: product of three normalized factors
    // - inlier ratio (data sufficiency)
    // - conditioning (geometric observability)
    // - residual sanity (didn't end up on garbage)
    ...
    const float c_cond = std::clamp(
        std::log10(std::max(res.cond_number, 1e-12f) / cfg_.min_cond_number) / 2.0f,
        0.0f, 1.0f);

    const float avg_r2 = res.inliers > 0
        ? res.final_residual / static_cast<float>(res.inliers) : 1e9f;
    const float c_resid = std::clamp(1.0f - (avg_r2 - 1.0f) / 8.0f, 0.0f, 1.0f);

    res.confidence = c_inlier * c_cond * c_resid;
    res.localized = (res.inliers >= cfg_.min_inliers) &&
                    (res.cond_number >= cfg_.min_cond_number) && 
                    (res.confidence > 0.0f);
    ...
    std::vector<Correspondence>& out) 
{
    ...
    for (size_t i = 0; i < surfels_sensor.size(); ++i) {
        ...
        const Eigen::Matrix3f S_f_w = R * fs.C_shape * R.transpose();
        ...
        for (const auto& o : kNb7) {
            ...
            for (const MapSurfel& ms : *v) {
                ...
                if (score < best_score) {
                    ...
                    best_sigma_n = std::sqrt(std::max(sigma2_n, 1e-10f));
                }
            }
        }
    ...
    float& cond_out) 
{
    ...
    cond_out = evals.minCoeff() / std::max(evals.maxCoeff(), 1e-12f);

    // Solve H * dx = -b
    Eigen::LDLT<Eigen::Matrix<float,6,6>> ldlt(H);
